...
const SCalendar = () => {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      try {
        // ✅ Call the new API
        const fetchedRes = await fetchAllClubEvents();

        // Safely extract the array depending on backend response format
        const eventsArray = Array.isArray(fetchedRes) ? fetchedRes : (fetchedRes?.data || []);

        // ✅ Map database fields to FullCalendar format
        setEvents(eventsArray.map((ev, i) => ({
          id: ev.event_id,
          title: ev.event_name,
          start: ev.start_date, // FullCalendar uses 'start'
          end: ev.end_date,     // FullCalendar uses 'end'
          extendedProps: {
            venue: ev.location,
            type: ev.event_type
          },
  ...
  // 🗑️ DELETED the redundant handleLogout function! (SNavbar handles this now)
                ...
                <div className="sc-agenda-list">
                  {agendaEvents.length > 0 ? agendaEvents.map((ev, i) => (
                    <div className="sc-agenda-item" key={i}>
                      <div className="sc-agenda-line" style={{ background: ev.color }}></div>
                      <div>
                        {/* ✅ Now pulling dynamic venue from extendedProps */}
                        <p className="sc-agenda-time">{ev.extendedProps?.type || "Event"}</p>
                        <p className="sc-agenda-name">{ev.title}</p>
                        <p className="sc-agenda-meta"><span className="material-symbols-outlined">location_on</span> {ev.extendedProps?.venue || "TBA"}</p>
                      </div>
                    </div>
                  )) : (
                    <div className="sc-agenda-empty-state">
                  ...
                  {upcoming.length > 0 ? upcoming.map((ev, i) => {
                    ...
                    return (
                      <div className="sc-upcoming-card group" key={i}>
                        ...
                        <div className="sc-up-info">
                          <p className="sc-up-title">{ev.title}</p>
                          <p className="sc-up-due">In {daysLeft} days</p>
                        </div>
                        <span className="material-symbols-outlined sc-up-arrow group-hover:translate-x-1">chevron_right</span>
                      </div>
                    );
                  }) : (
                    <p className="sc-upcoming-empty">No upcoming events!</p>
                  )}
                </div>
              </section>
            </div>
