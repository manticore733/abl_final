...
import { fetchStudentInfo, submitStudentEntry } from "../../api/studentApi";
...
// Points Mapping (Kept exactly as is)
const pointsMapping = {
  "Technical Seminar - Attending/Organizing": 1,
  "Technical Seminar - Delivering": 2,
  "Technical Workshop - Attending": 2,
...
const SMakeEntry = () => {
  ...
  useEffect(() => {
    // ✅ Fetch student details securely using the new API service
      ...
      try {
        ...
        if (studentData) {
          setFormData((prev) => ({
            ...
            roll_number: studentData.s_username,
            name: studentData.s_name, // Fixed to match database field 's_name'
  ...
  // 🗑️ Removed redundant handleLogout (SNavbar handles it)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ Use the centralized API call to submit data
      await submitStudentEntry(formData);
      showToast('success', 'Success', 'Activity submitted successfully!');

      // Reset form but keep identity data intact
      setFormData((prev) => ({
        ...prev, event_name: "", event_type: "", subcategory: "",
        sub_activity_type: "", organised_by: "", participation_date: "",
        venue: "", allocated_points: 0,
      ...
      console.error("Error submitting activity:", error);
      showToast('error', 'Server Error', 'Failed to submit activity. Please try again.');
    }
  };
  ...
  return (
    <div className="sme-page-wrapper">
      ...
      <main className="sme-main-content">
        ...
        <div className="sme-form-card editorial-shadow">
          <form onSubmit={handleSubmit}>
            ...
            <section className="sme-form-section no-border">
              ...
              <div className="sme-grid-entry">
                <div className="sme-input-group col-span-2">
                  <label htmlFor="event_name">Event Name {errors.event_name && <span className="sme-error">({errors.event_name})</span>}</label>
                  <input type="text" id="event_name" name="event_name" placeholder="e.g., Global Tech Symposium 2024" className={`sme-input ${errors.event_name ? 'input-error' : ''}`} value={formData.event_name} onChange={handleChange} />
                </div>

                <div className="sme-input-group">
                  <label htmlFor="event_type">Event Type {errors.event_type && <span className="sme-error">({errors.event_type})</span>}</label>
                  <div className="sme-select-wrapper">
                    <select id="event_type" name="event_type" className={`sme-input ${errors.event_type ? 'input-error' : ''}`} value={formData.event_type} onChange={handleChange}>
                      <option value="">Select Category</option>
                      {Object.keys(categoryOptions).map((type) => (<option key={type} value={type}>{type}</option>))}
                    </select>
                    <span className="material-symbols-outlined sme-select-icon">expand_more</span>
                  </div>
                </div>
                ...
                <div className="sme-input-group">
                  <label htmlFor="subcategory">Subcategory {errors.subcategory && <span className="sme-error">({errors.subcategory})</span>}</label>
                  <div className="sme-select-wrapper">
                    <select id="subcategory" name="subcategory" className={`sme-input ${errors.subcategory ? 'input-error' : ''}`} value={formData.subcategory} onChange={handleChange} disabled={!formData.event_type}>
                      <option value="">Select Subcategory</option>
                      {categoryOptions[formData.event_type]?.map((event) => (<option key={event} value={event}>{event}</option>))}
                    </select>
                    <span className="material-symbols-outlined sme-select-icon">expand_more</span>
                  </div>
                </div>
                ...
                <div className="sme-input-group">
                  <label htmlFor="sub_activity_type">Sub Activity Type {errors.sub_activity_type && <span className="sme-error">({errors.sub_activity_type})</span>}</label>
                  <div className="sme-select-wrapper">
                    <select id="sub_activity_type" name="sub_activity_type" className={`sme-input ${errors.sub_activity_type ? 'input-error' : ''}`} value={formData.sub_activity_type} onChange={handleChange} disabled={!subCategoryOptions[formData.subcategory]}>
                      <option value="">Select Activity Type</option>
                      {(subCategoryOptions[formData.subcategory] || []).map((sub) => (<option key={sub} value={sub}>{sub}</option>))}
                    </select>
                    <span className="material-symbols-outlined sme-select-icon">expand_more</span>
                  </div>
                </div>
                ...
                <div className="sme-input-group">
                  <label htmlFor="organised_by">Organised By {errors.organised_by && <span className="sme-error">({errors.organised_by})</span>}</label>
                  <div className="sme-select-wrapper">
                    <select id="organised_by" name="organised_by" className={`sme-input ${errors.organised_by ? 'input-error' : ''}`} value={formData.organised_by} onChange={handleChange}>
                      <option value="">Organization Name</option>
                ...
                <div className="sme-input-group">
                  <label htmlFor="participation_date">Participation Date {errors.participation_date && <span className="sme-error">({errors.participation_date})</span>}</label>
                  <input type="date" id="participation_date" name="participation_date" className={`sme-input ${errors.participation_date ? 'input-error' : ''}`} value={formData.participation_date} onChange={handleChange} />
                </div>

                <div className="sme-input-group">
                  <label htmlFor="venue">Venue {errors.venue && <span className="sme-error">({errors.venue})</span>}</label>
                  <input type="text" id="venue" name="venue" placeholder="Physical or Virtual Location" className={`sme-input ${errors.venue ? 'input-error' : ''}`} value={formData.venue} onChange={handleChange} />
                </div>
              ...
              <div className="sme-form-actions">
                <button type="button" className="sme-btn-draft" onClick={() => showToast('info', 'Draft Saved', 'Your activity progress has been saved locally.')}>Save as Draft</button>
                <button type="submit" className="sme-btn-submit"><span>Submit for Review</span><span className="material-symbols-outlined">send</span></button>
              </div>
            </section>
        ...
        <aside className="sme-info-cards">
          {/* Static info cards remain exactly the same */}
          <div className="sme-info-card">
            <div className="sme-info-icon bg-orange-light text-orange"><span className="material-symbols-outlined">info</span></div>
            <h3>Submission Guide</h3>
            <p>Ensure all certificates are attached in the next step. Late submissions might require department head approval.</p>
          </div>
          <div className="sme-info-card">
            <div className="sme-info-icon bg-primary-light text-primary"><span className="material-symbols-outlined">verified</span></div>
            <h3>Verification Timeline</h3>
            <p>Most activities are verified within 3-5 working days by your assigned mentor.</p>
          </div>
          <div className="sme-info-card">
            <div className="sme-info-icon bg-cyan-light text-cyan"><span className="material-symbols-outlined">stars</span></div>
            <h3>Credit Milestone</h3>
            <p>You are currently 45 points away from reaching the 'Gold' achievement tier for this semester.</p>
          </div>
        </aside>
