...
import './css/SProfile.css';
import profileImage from "../../assets/image.png";
import { Link } from "react-router-dom"; // Removed useNavigate & axios
...
const SProfile = () => {
  ...
  // Consolidated formData using only the secure fetchStudentInfo call
  const [formData, setFormData] = useState({
    s_id: "",
    roll_number: "",
    name: "",
      ...
      try {
        ...
        if (data) {
          setFormData({
            s_id: data.s_id,
            roll_number: data.s_username,
            name: data.s_name, // Fixed to map to the new database column
            department: data.department,
            division: data.division,
            semester: data.semester,
            batch: data.batch,
  ...
  useEffect(() => {
    ...
    activities.forEach((activity) => {
      // ✅ Updated to match new backend keys (event_type & allocated_points)
      // Only count points if the status is Approved!
      if (activity.status === "Approved" && categoryPoints.hasOwnProperty(activity.event_type)) {
        const points = activity.allocated_points || 0;
        categoryPoints[activity.event_type] += points;
        total += points;
      }
    });
  ...
  return (
    <div className="sp-page-wrapper">
      {/* ✅ REPLACED THE MASSIVE HARDCODED NAV WITH THE COMPONENT */}
      <SNavbar />
          ...
          <div className="sp-bento-grid">
            <div className="sp-panel sp-col-7">
              <div className="sp-panel-header-flex">
                <div>
                  <h2 className="sp-panel-title">Holistic Performance</h2>
                  <p className="sp-panel-subtitle">Activity Points Distribution (Approved Only)</p>
                </div>
                <div className="sp-percentile-pill">Top Contributor</div>
              </div>
            ...
            <div className="sp-table-wrapper">
              <table className="sp-ledger-table">
                ...
                <tbody>
                  {currentActivities.length > 0 ? (
                    currentActivities.map((act, index) => (
                      <tr key={act.activity_id} className="sp-ledger-row group">
                        <td className="sp-td-first sp-sr-cell">
                          {(page - 1) * itemsPerPage + index + 1}
                        </td>
                        <td>
                          <div className="sp-activity-cell">
                            <div className={`sp-act-icon icon-${act.event_type?.toLowerCase() || 'default'}`}>
                              <span className="material-symbols-outlined">{getIconForType(act.event_type)}</span>
                            </div>
                            <div>
                              {/* ✅ Fixed data mapping to new database keys */}
                              <div className="sp-act-title">{act.event_name}</div>
                              <div className="sp-act-subtitle">{act.subcategory || act.event_type}</div>
                            </div>
                          </div>
                        </td>
                        <td>
                          {/* Use the status for classification pill since we dropped 'level' in phase 1 */}
                          <span className={`sp-level-pill sp-pill-uni`}>
                            {act.sub_activity_type || "General"}
                          </span>
                        </td>
                        <td className="sp-act-contribution">
                          {formatDate(act.participation_date)}
                          <br /><span className="sp-venue-text">{act.venue || "N/A"}</span>
                        </td>
                        <td className="sp-td-last text-right">
                          <span className={`sp-act-points ${act.status === "Approved" ? "text-approved" : "text-pending"}`}>
                            {act.status === "Approved" ? `+${act.allocated_points}` : "Pending"}
                          </span>
                        </td>
                      </tr>
                    ))
