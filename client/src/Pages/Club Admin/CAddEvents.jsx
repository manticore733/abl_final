...
const CAddEvents = () => {
  ...
  // Extract editMode and event details passed from CProfile.jsx
  const editMode = location.state?.editMode || false;
  const eventDetails = location.state?.eventDetails || null;

  const [formData, setFormData] = useState({
    roll_number: sessionStorage.getItem("username") || "",
    club_name: sessionStorage.getItem("club_name") || "",
    admin_name: "",
    event_name: "",
    description: "",
    start_date: "",
  ...
  useEffect(() => {
    if (editMode && eventDetails) {
      setFormData({
        roll_number: eventDetails.roll_number || sessionStorage.getItem("username"),
        club_name: eventDetails.club_name || sessionStorage.getItem("club_name"),
        admin_name: eventDetails.admin_name || "",
        event_name: eventDetails.event_name || "",
        description: eventDetails.description || "",
        start_date: eventDetails.start_date || "",
        end_date: eventDetails.end_date || "",
        start_time: eventDetails.start_time ? eventDetails.start_time.substring(0, 5) : "", // Format to HH:MM
        end_time: eventDetails.end_time ? eventDetails.end_time.substring(0, 5) : "",
        ...
        poster_image: null,
        poster_preview: eventDetails.poster_image ? `http://localhost:5000/${eventDetails.poster_image.replace("\\", "/")}` : null,
      });
    }
  ...
  const validateForm = () => {
    ...
    // Only check past dates if we are creating a NEW event. If editing, the start date might legitimately be in the past!
    if (!editMode && formData.start_date && formData.start_date < today) newErrors.start_date = "Cannot be before today.";
    ...
    if (!formData.event_mode) newErrors.event_mode = "Please select a mode.";
    if (!formData.event_type) newErrors.event_type = "Please select a type.";
    if (!editMode && !formData.poster_image) newErrors.poster_image = "Poster image is required.";
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validateForm()) {
      showToast('error', 'Validation Error', 'Please fix the highlighted errors before saving.');
      return;
    }
    ...
    try {
      if (editMode) {
        // --- EDIT EXISTING EVENT ---
        await axios.put(`http://localhost:5000/api/clubadmin/events/${eventDetails.event_id}`, data, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        showToast('success', 'Event Updated!', 'The scholarly details have been successfully modified.');
        navigate('/cprofile'); // Send them back to the dashboard table!
      } else {
        // --- CREATE NEW EVENT ---
        const response = await addClubEvent(data);
        showToast('success', 'Event Published!', response.message || "Your event has been successfully published.");
        setFormData({
          roll_number: sessionStorage.getItem("username") || "", club_name: sessionStorage.getItem("club_name") || "",
          admin_name: "", event_name: "", description: "", start_date: "", end_date: "", start_time: "",
          end_time: "", location: "", entry_fee: "", event_type: "", event_mode: "", event_link: "",
          poster_image: null, poster_preview: null,
        });
      }
      setErrors({});
    } catch (error) {
      showToast('error', editMode ? 'Update Failed' : 'Publish Failed', error.response?.data?.message || "Failed to process request. Please try again.");
    }
  };
        ...
        <div className="ca-form-container">
          <header className="ca-form-header">
            <nav className="ca-breadcrumbs">
              ...
              <span className="material-symbols-outlined">chevron_right</span>
              {/* DYNAMIC BREADCRUMB */}
              <span className="current">{editMode ? "Edit Campaign" : "New Campaign"}</span>
            </nav>
            {/* DYNAMIC TITLE */}
            <h1>{editMode ? "Edit Event Details" : "Create New Event"}</h1>
            <p>
              {editMode
                ? "Update the scholarly details for your upcoming gathering. Changes reflect immediately."
                : "Set up your next campus initiative with curated editorial precision."}
            </p>
          </header>
          ...
          <form className="ca-form" onSubmit={handleSubmit}>
            ...
            <section className="ca-form-section">
              ...
              <div className="ca-grid-2">
                ...
                <div className="ca-input-group">
                  <label>Roll Number</label>
                  <input type="text" id="roll_number" value={formData.roll_number} disabled className="opacity-70 cursor-not-allowed" />
                </div>
                <div className="ca-input-group full-width">
                  <label>Club Name</label>
                  <input type="text" id="club_name" value={formData.club_name} disabled className="opacity-70 cursor-not-allowed" />
                </div>
              </div>
            </section>
            ...
            <div className="ca-form-actions">
              <button
                ...
                onClick={() => editMode ? navigate('/cprofile') : window.location.reload()}
              >
                {editMode ? "Cancel Changes" : "Clear Form"}
              </button>

              {/* DYNAMIC BUTTON TEXT */}
              <button type="submit" className="ca-btn-publish">
                {editMode ? "Save & Update Event" : "Publish Event"} <span className="material-symbols-outlined">{editMode ? 'save' : 'send'}</span>
              </button>
            </div>
