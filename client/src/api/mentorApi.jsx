...
export const FetchStudentList = async () => {
  try {
    const response = await apiClient.get("/api/mentor/students");
...
export const FetchStudentActivities = async (studentId) => {
  const response = await apiClient.get(`/api/mentor/pending-activities/${studentId}`);
  return response.data;
};

export const AssignActivityPoints = async (activityId, pointsData) => {
  const response = await apiClient.put(`/api/mentor/assign-points/${activityId}`, pointsData);
  return response.data;
};

export const approveActivity = async (activityId) => {
  return await apiClient.post(`/api/mentor/approve/${activityId}`);
};