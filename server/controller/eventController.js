...
export const getAllClubEvents = async (req, res) => {
  try {
    // We pass the protocol (http/https) and host (localhost:5000) to the service 
    // so it can build the perfect image URLs for the frontend!
    const events = await eventService.getFormattedPublicEvents(req.protocol, req.get("host"));

    // Return direct array for frontend map() compatibility
    ...
    return sendResponse(res, 500, false, null, "Failed to fetch club events.");
  }
};

// If you have a specific route expecting 'fetchEvents', alias it to avoid breaking the UI immediately
export const fetchEvents = getAllClubEvents;
export const fetchEvents2 = getAllClubEvents;