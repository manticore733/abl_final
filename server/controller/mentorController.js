...
export const fetchMentorCreds = async (req, res) => {
  try {
    const mentorId = req.user.id; // From JWT Middleware!
    ...
    if (!mentorDetails) return sendResponse(res, 404, false, null, "Mentor not found.");

    return sendResponse(res, 200, true, mentorDetails);
  } catch (error) {
    console.error("🔥 Error fetching mentor profile:", error);
...
export const fetchMentorProfile = fetchMentorCreds; // Alias to keep routes working
...
export const getMentorCredentials = async (req, res) => {
  try {
    ...
    const mentor = await MentorInfo.findOne({
      ...
      attributes: ["m_username", "m_password"],
    });
    if (!mentor) return res.status(404).json({ error: "Mentor not found" });
    return res.status(200).json(mentor);
  } catch (error) {
    return res.status(500).json({ error: "Failed to fetch mentor credentials" });
  }
};
...
export const fetchStudentsUnderMentor = async (req, res) => {
  try {
    ...
    const students = await mentorService.getAssignedStudents(mentorId);
    ...
    if (error.message === "MENTOR_NOT_FOUND") {
      return sendResponse(res, 404, false, null, "Mentor profile not found.");
    }
...
// Aliases for overlapping frontend calls
export const fetchStudentDetails = fetchStudentsUnderMentor;
export const fetchAllStudentsForMentor = fetchStudentsUnderMentor;
...
export const fetchPendingActivities = async (req, res) => {
  try {
    const { studentId } = req.params;
    const activities = await StudentActivitySummary.findAll({
      where: { s_id: studentId, status: "Pending" },
    });

    if (!activities.length) return res.status(404).json({ message: "No pending activities found." });
...
export const fetchProcessedActivities = async (req, res) => {
  try {
    const { studentId } = req.params;
    const activities = await StudentActivitySummary.findAll({
      where: { s_id: studentId, status: ["Approved", "Rejected"] },
    });

    if (!activities.length) return res.status(404).json({ message: "No processed activities found." });
    return res.status(200).json(activities);
  } catch (error) {
    return res.status(500).json({ error: "Internal server error." });
  }
...
export const getStudent = async (req, res) => {
  // Legacy route: fetches all activities regardless of status
  try {
    const { student_id } = req.params;
    const activities = await StudentActivitySummary.findAll({ where: { s_id: student_id } });
    return res.status(200).json(activities);
  } catch (error) {
...
// ==========================================
// ✅ APPROVE / REJECT / UNDO (The Services!)
// ==========================================

export const approveActivity = async (req, res) => {
  try {
    const { activityId } = req.params;
    await mentorService.approveStudentActivity(activityId);
    return res.status(200).json({ message: "Activity approved successfully!" });
  } catch (error) {
    return res.status(400).json({ error: "Activity not found or already processed." });
  }
};
...
export const rejectActivity = async (req, res) => {
  try {
    const { activityId } = req.params;
    const { rejectionReason } = req.body;
    await mentorService.rejectStudentActivity(activityId, rejectionReason);
    return res.status(200).json({ message: "Activity rejected successfully!" });
  } catch (error) {
    return res.status(400).json({ error: "Activity not found or already processed." });
  }
};
...
export const undoActivityAction = async (req, res) => {
  try {
    ...
    if (activity.status === "Pending") return res.status(400).json({ error: "Activity is already pending." });

    if (activity.status === "Approved") {
      const student = await StudentDetails.findOne({ where: { s_id: activity.s_id } });
      if (student) {
        student.total_credits -= activity.allocated_points;
        await student.save();
      }
    }
    ...
    return res.status(200).json({ message: "Activity status reverted to Pending." });
  } catch (error) {
    return res.status(500).json({ error: "Internal server error." });
  }
};
...
export const assignPoints = async (req, res) => {
  try {
    ...
    const points = await mentorService.calculateAndAssignPoints(a_id, a_type, a_sub_type, a_level);
    return res.status(200).json({ message: "Points assigned successfully!", points });
  } catch (error) {
    return res.status(500).json({ error: "Failed to assign points" });
  }
...
export const fetchStudentStatistics = async (req, res) => {
  try {
    ...
    if (!activities.length) return res.status(404).json({ message: "No activities found." });

    let eventCounts = { Technical: 0, Cultural: 0, Sports: 0, Social: 0, Internship: 0 };
    let statusCounts = { Approved: 0, Pending: 0, Rejected: 0 };
    let semesterWiseEvents = {};

    activities.forEach(({ semester, event_type, status }) => {
      if (event_type in eventCounts) eventCounts[event_type]++;
      if (status in statusCounts) statusCounts[status]++;

      const semKey = semester || "Unknown";
      if (!semesterWiseEvents[semKey]) {
        semesterWiseEvents[semKey] = { Technical: 0, Cultural: 0, Sports: 0, Social: 0, Internship: 0 };
      }
      if (event_type in semesterWiseEvents[semKey]) {
        semesterWiseEvents[semKey][event_type]++;
      }
    });

    return res.status(200).json({ eventCounts, statusCounts, semesterWiseEvents });
  } catch (error) {
    console.error("🔥 Error fetching student statistics:", error);
    return res.status(500).json({ error: "Internal server error." });
  }
...
export const generateMentorReport = async (req, res) => {
  try {
    ...
    const { buffer, filename } = await reportService.generateStudentDocxReport(rollNumber, barChartBase64);
